...
public class IslandController : MonoBehaviour
{
    ...
    void Update()
    {
        if (_isEmerging)
        {
            ...
            if (_rumble)
            {
                float distanceFromOrigin = Mathf.Abs(_meshRenderer.transform.localPosition.y - _originalLocalPosition.y);
                distanceFromOrigin = Mathf.Clamp01(distanceFromOrigin);
                float xDirection = Random.Range(-distanceFromOrigin, distanceFromOrigin);
                float zDirection = Random.Range(-distanceFromOrigin, distanceFromOrigin);
                float xNoise = Mathf.PerlinNoise(_rumbleSpeed * Time.time, 0);
                float zNoise = Mathf.PerlinNoise(0f, _rumbleSpeed * Time.time);

                noise.x = xDirection * xNoise;
                noise.z = zDirection * zNoise;
            }
